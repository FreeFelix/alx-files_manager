...
export default class FilesController {
  ...
  static async postUpload(req, res) {
    ...
    // Write file to local path if it is not a folder
    ...
    // Start thumbnail generation worker if the file is an image
    if (type === VALID_FILE_TYPES.image) {
      const jobName = `Image thumbnail [${userId}-${fileId}]`;
      fileQueue.add({ userId, fileId, name: jobName });
    }

    // Return the file metadata as response
    res.status(201).json({
      id: fileId,
      userId,
      name,
  ...
  static async putPublish(req, res) {
    ...
    // Update the file document to set it as public
    await (await dbClient.filesCollection())
      .updateOne(fileFilter, { $set: { isPublic: true } });

    // Return the updated file metadata as response
    res.status(200).json({
      id,
      userId,
      name: file.name,
  ...
  static async putUnpublish(req, res) {
    ...
    // Update the file document to set it as private
    await (await dbClient.filesCollection())
  ...
  static async getFile(req, res) {
    ...
    // Serve the file content with appropriate headers
